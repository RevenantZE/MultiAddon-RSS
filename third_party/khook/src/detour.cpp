...
std::atomic<bool> g_TerminateWorker{false};
std::mutex g_WorkerStartMutex;
bool g_WorkersStarted = false;
std::thread g_InsertThread;
std::thread g_DeleteThread;

static void InsertWorkerMain() {
...
// Standalone plugins must not launch workers merely by being dlopened.
static void EnsureWorkersStarted() {
	std::lock_guard guard(g_WorkerStartMutex);
	if (g_WorkersStarted)
		return;
	g_TerminateWorker = false;
	g_InsertThread = std::thread(InsertWorkerMain);
	g_DeleteThread = std::thread(DeleteWorkerMain);
...
KHOOK_API void Shutdown(
	...
	std::lock_guard worker_guard(g_WorkerStartMutex);
	g_TerminateWorker = true;
	if (g_InsertThread.joinable())
		g_InsertThread.join();
	if (g_DeleteThread.joinable())
		g_DeleteThread.join();
	...
	std::vector<HookID_t> ids;
	{
		...
		for (const auto& entry : g_associated_hooks)
	...
	{
		std::lock_guard guard(g_insert_hooks_mutex);
		g_insert_hooks.clear();
	}
	{
		std::lock_guard guard(g_delete_hooks_mutex);
		g_delete_hooks.clear();
	}
	{
		std::lock_guard guard(g_associated_hooks_mutex);
		g_associated_hooks.clear();
	}
	{
		std::lock_guard guard(g_hooks_detour_mutex);
...
// Constructed after this translation unit's containers and destroyed before
// them. Covers factory probing and partial Load failure as well as Unload.
struct StandaloneShutdownGuard {
	~StandaloneShutdownGuard() { Shutdown(); }
};
static StandaloneShutdownGuard g_StandaloneShutdownGuard;
